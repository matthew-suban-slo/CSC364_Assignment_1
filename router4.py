...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    # Connect to next-hop routers if needed (example: router 5 and 6)
    try:
        soc_5 = create_socket('127.0.0.1', 8005)
    except:
        soc_5 = None
    try:
        soc_6 = create_socket('127.0.0.1', 8006)
    except:
        soc_6 = None
    ...
    while True:
        ...
        if sending_port == '8005' and soc_5:
            if new_ttl <= 0:
                ...
                soc_5.send(new_packet.encode())
        elif sending_port == '8006' and soc_6:
            if new_ttl <= 0:
                print("DISCARD:", new_packet)
                write_to_file("output/discarded_by_router_4.txt", new_packet)
            else:
...
if __name__ == "__main__":
    # Remove any output files in the output directory (optional, for clean runs)
